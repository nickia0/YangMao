...
!(async () => {
  if (process.env.DKD_HD&& process.env.DKD_HD.indexOf('#') > -1) {
   dkdhd = process.env.DKD_HD.split('#');
   console.log(`您選擇的是用"#"隔開\n`)
  }
  else if (process.env.DKD_HD && process.env.DKD_HD.indexOf('\n') > -1) {
   dkdhd = process.env.DKD_HD.split('\n');
  ...
  if (process.env.DKD_BD&& process.env.DKD_BD.indexOf('#') > -1) {
   dkdbody = process.env.DKD_BD.split('#');
   console.log(`您選擇的是用"#"隔開\n`)
  }
  else if (process.env.DKD_BD && process.env.DKD_BD.indexOf('\n') > -1) {
   dkdbody = process.env.DKD_BD.split('\n');
   console.log(`您選擇的是用換行隔開\n`)
  } else {
   dkdbody = process.env.DKD_BD.split()
  ...
  if (process.env.DKD_VBD&& process.env.DKD_VBD.indexOf('#') > -1) {
   bodys = process.env.DKD_VBD.split('#');
   console.log(`bodys您選擇的是用"#"隔開\n`)
  }
  else if (process.env.DKD_VBD && process.env.DKD_VBD.indexOf('\n') > -1) {
   bodys = process.env.DKD_VBD.split('\n');
